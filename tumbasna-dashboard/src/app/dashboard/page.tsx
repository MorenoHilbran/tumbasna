...
export default function DashboardPage() {
    ...
    return (
        <div className="p-4 md:p-8 space-y-8 bg-[#F8FAFC]">
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-200/60 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        <Banknote className="w-5 h-5 text-emerald-600" />
                        <h2 className="text-base font-bold text-slate-900 tracking-tight">Pendapatan Platform</h2>
                    </div>
                    <p className="text-xs text-slate-400">Dari Biaya Layanan (Rp 2.000 / Transaksi Selesai)</p>
                </div>
                
                <div className="flex flex-wrap items-center gap-6 md:gap-10">
                    <div>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Hari Ini</p>
                        <p className="text-lg font-extrabold text-slate-800">
                            Rp {((stats?.dailyTransactions?.[6]?.value || dailyTransactions[6].value) * 2000).toLocaleString('id-ID')}
                        </p>
                    </div>
                    <div className="hidden sm:block w-px h-10 bg-slate-100"></div>
                    <div>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Bulan Ini</p>
                        <p className="text-lg font-extrabold text-slate-800">
                            Rp {((stats?.kpi?.totalTransactions || 12847) * 0.15 * 2000).toLocaleString('id-ID')}
                        </p>
                    </div>
                    <div className="hidden sm:block w-px h-10 bg-slate-100"></div>
                    <div>
                        ...
                        <div className="flex items-center gap-2">
                            <div className="px-2 py-0.5 rounded bg-emerald-50 text-emerald-600 flex items-center">
                                <Coins className="w-3.5 h-3.5 mr-1" />
                            </div>
                            <p className="text-xl font-black text-emerald-600 tracking-tight">
                                Rp {((stats?.kpi?.totalTransactions || 12847) * 2000).toLocaleString('id-ID')}
                            </p>
                        </div>
                    </div>
                </div>
